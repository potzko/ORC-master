...
class parser:
    ...
    def let_statement(self):
        ...
        match self.lookahead[0]:
            case ';':
                self.eat(';')
                return ('let', id, ('literal_num', '0', 'u8'), type)
            case 'op_=':
                self.eat('op_=')
                return ('let', id, self.expression_statement(), type)
    ...
    def literal(self):
        ...
        match self.lookahead[0]:
            case 'literal_boolean':
                return ('literal_num', 1 if self.eat('literal_boolean') == 'true' else 0, 'u8')
            case 'literal_string':
            ...
            case 'literal_num':
                ...
                match self.lookahead[0]:
                    case ':':
                        self.eat(':')
                        return ('literal_num', num, self.eat('identifier'))
                    case _:
                        return ('literal_num', num, 'u8')
            case _:
                raise Exception(f"expected literal found {self.lookahead[0]}")
